/**
 * ui-render.js - Gestión de Grid y Renderizado con Soporte de Auditoría
 */
...
export function renderizarUnidades(unidades) {
    ...
    // No permitimos agregar nuevas unidades si la jornada ya terminó
    if (!jornadaCerrada) {
        const cardAdd = document.createElement('div');
        cardAdd.className = 'card-add-unit';
        cardAdd.innerHTML = `<input type="text" id="input-nueva-unidad" placeholder="ID + ENTER" aria-label="Agregar unidad">`;
...
function crearCardUnidad(data) {
    ...
    // Aplicamos la clase de bloqueo que definimos en SCSS
    if (jornadaCerrada) card.classList.add('jornada-bloqueada');
    ...
    if (data.observaciones && data.observaciones.trim() !== "") {
        ...
        badgeObs.innerHTML = '📝';
        badgeObs.title = "Tiene notas registradas";
    ...
    clone.querySelectorAll('.btn-banda').forEach(btn => {
        ...
        btn.onclick = async (e) => {
            e.stopPropagation();
            if (jornadaCerrada) return; // Bloqueado por pointer-events en CSS, pero validamos aquí también
    ...
    if (data.esCampo) btnCampo.classList.add('active');
    ...
    // Botón Editar (Notas) - SIEMPRE HABILITADO
